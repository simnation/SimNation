...
public class SimNationLauncher extends DirectLauncher {
	...
	public static void main(String args[]) {
		...
		final RootDomain model=launcher.loadScenario("E:\\git\\SimNation\\scenario","HouseholdTest","","",true);
		if (model!=null) {
	...
	private RootDomain loadScenario(String path,String database,String user,String pwd,boolean logging) {
		DataAccessObject dao=null;
		RootDomain model=null;

		try {
			dao=new DataAccessObject(DB_TYPE.H2,path,database);
			Root.getInstance().load(dao);
			model=new RootDomain(dao);
		}
