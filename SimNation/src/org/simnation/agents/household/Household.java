...
public final class Household extends AbstractBasicAgent<HouseholdState,Household.EVENT> {
	...
	private void planBudget() {
		...
		for (final URGENCY urgency : URGENCY.values())
			...
			// level
			for (final Need iter : urgencyMap.get(urgency)) {
			...
			else for (final Need iter : urgencyMap.get(urgency)) {
				deactivateNeed(iter);
				getState().setBudget(iter,0);
			}
	}
	...
	// idee: personality
	private void sendDemand(Need nt) {
	...
	private float getMinQualtity(Need nt) {
		// 0: reiner Entscheid �ber Marktpreise
		return 0;
	}
	
	// used for price mark-up
	private double calcUrgency(Need nd, Time time) {
		if (getFrustrationTime()==Time.INFINITY) return 1.0d;
		final double activityPeriod=nd.getFrustrationTime()-nd.getActivationTime();
		final double actualPeriod=getFrustrationTime().getTicks()-time.getTicks();
	...
	private void activateNeed(Need nd) {
	...
	private void deactivateNeed(Need nd) {
	...
	private Batch getBatchFromCart(Need nt) {
